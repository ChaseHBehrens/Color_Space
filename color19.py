...
for i in range(round(n)):
    ...
    for point in points:
        colors.append([100 * (((i/n) + (1/(2*n))) ** 0.9), 280 * (point[0]/100) ** 1.2, ((i%2)*(i+1)*30) + offset + math.degrees(point[1])])
...
class Camera:
    ...
    def update_offset(self, speed):
        ...
        for i in range(round(n)):
            if i%2:
                ...
                points = hex_circle_packing((100/n), 100, True)
            for point in points:
                colors.append([100 * (((i/n) + (1/(2*n))) ** 0.9),180 * (point[0]/100) ** 1.2, (((i+1)%2)*i*30) + offset + math.degrees(point[1])])
    ...
    def calcuate_point(self, point):
        ...
        rotated_point = [5 * (rotated_point[1] ** (1 / 1.2)) * math.cos(math.radians(rotated_point[2])), 
                         2.5 * (75 - (rotated_point[0] ** (1 / 0.9))), 
                         5 * (rotated_point[1]  ** (1 / 1.2)) * math.sin(math.radians(rotated_point[2])),]
